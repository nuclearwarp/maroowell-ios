...
final class MetaRealtimeStore: ObservableObject {
    ...
    private static func build(
        ...
        for source in current where isStarted(source) {
            ...
            let delivery = Self.delivery(source, prefix: "delivery")
            let freshDelivery = Self.delivery(source, prefix: "fresh_delivery")
            let returns = Self.collection(source, prefix: "return", includeAbsent: true)
            let freshbag = Self.collection(source, prefix: "freshbag", includeAbsent: false)
